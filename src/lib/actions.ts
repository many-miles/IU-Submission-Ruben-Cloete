...
export async function createPitch(
  ...
  try {
    ...
    try {
      ...
      console.warn("services.json not found or invalid, creating new file:", error);
    }
...
/**
 * Server Action: Update service view count
 * 
 * Increments the view counter for a specific service when users view the detail page.
 * This helps track service popularity.
 * 
...
export async function incrementServiceViews(serviceId: string) {
  try {
    ...
    const serviceIndex = data.services.findIndex((s) => s._id === serviceId);
    
    if (serviceIndex !== -1) {
      data.services[serviceIndex].views = (data.services[serviceIndex].views || 0) + 1;
      await writeFile(filePath, JSON.stringify(data, null, 2), "utf8");
    }
