...
// Service Filtering by Distance
import { ServiceTypeCard } from "../types/service";

// Filter an array of services based on proximity to a user's location
export function filterServicesByDistance(
  ...
  return services
    .map(service => ({
      ...service,
      // Compute distance if service has a defined location
      distance: service.location 
        ? calculateDistance(userLocation, service.location)
        : null
    }))
    // Only include services within the maximum allowed distance
    .filter(service => service.distance !== null && service.distance <= maxDistance)
    // Sort ascending by distance for convenience in UI
...
/** 
 * Geofencing / Area Check
 * 
 * Check if a given location falls within approximate bounds of Jeffreys Bay
 */ 
