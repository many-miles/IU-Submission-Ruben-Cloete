...
const allServices: MapService[] = servicesData.services
  .filter((s): s is typeof s & { location: { lat: number; lng: number } } => !!s.location)
  .map((s) => ({
    _id: s._id,
    title: s.title || null,
    category: (s.category as string | null) || null,
    location: s.location, // 
    priceRange: (s.priceRange as any) || null,
    author: s.author
      ? {
          _id: s.author._id,
    ...
    <>
      ...
      <section className="section_container">
        ...
        <div className="grid md:grid-cols-3 gap-4 mt-8">
          ...
          {Array.from(
            new Set(servicesData.services.map((s) => s.category).filter(Boolean)) // Unique categories
          ).map((category) => {
        ...
        <div className="mt-8 bg-blue-50 border-l-4 border-primary p-4 rounded">
          <p className="text-sm text-blue-800">
            💡 <strong>Tip:</strong> Click on any map marker to see service details and contact information.
          </p>
        </div>
      </section>
    </>
