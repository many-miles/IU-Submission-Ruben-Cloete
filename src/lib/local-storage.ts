...
/** 
 *Local Storage Services Utilities
 * 
 * Retrieve locally stored services array
 */
export const getStoredServices = () => {
  if (typeof window === 'undefined') return [];
  const services = localStorage.getItem('localServices');
  return services ? JSON.parse(services) : [];    // Return empty array if none
};

import { ServiceTypeCard } from '@/types/service';

// Add a new service to localStorage
export const addStoredService = (service: ServiceTypeCard) => {
  if (typeof window === 'undefined') return;
  const services = getStoredServices();          // Get existing services
  services.push(service);                         // Add new service
  localStorage.setItem('localServices', JSON.stringify(services)); // Save
  return service;                                 // Return added service
};
